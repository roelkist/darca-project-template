...
ifdef CI
    ...
    export VENV_PATH := /tmp/darca-module_name-venv
    export POETRY_HOME := /tmp/poetry-module_name-cache
    export POETRY_CONFIG_DIR := /tmp/poetry-module_name-config
    export POETRY_CACHE_DIR := $(POETRY_HOME)
    export POETRY_VIRTUALENVS_PATH := $(VENV_PATH)
    export PYTHONPYCACHEPREFIX := /tmp/module_name-pycache
    ...
    export PRE_COMMIT_HOME := /tmp/precommit-module_name-cache
endif
